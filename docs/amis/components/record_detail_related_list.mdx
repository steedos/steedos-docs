---
title: 相关表
description: 华炎魔方微页面引擎，内置相关表组件 RecordDetailRelatedList，借助amis渲染器，可以帮助开发人员快速生成一个功能完善的相关表视图组件。
---

这是一个华炎魔方相关表视图组件，一般用于在记录详情界面显示相关子表列表，它把对象相关表记录显示为表格，同时包含了对象记录的增删改查以及其他操作按钮功能。
...
```
如果我们按照教程 [Amis微页面 - 记录页面](/docs/amis/page-type#%E8%AE%B0%E5%BD%95%E9%A1%B5%E9%9D%A2) 新建一个指向业务伙伴 accounts 对象的微页面，并拖动一个相关表组件到画布，可以看到该组件生成了类似上面的amis Schema配置。
...
所有的相关表组件都监听了对象的[`@data.change`事件](/docs/amis/component#@data.change事件)，只要触发对象的`@data.change`事件，就能间接调用该组件的以下动作： 
- [刷新](#刷新)
