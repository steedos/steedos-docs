...
The translation content changes the Chinese display name of the "公司" object to "总公司", changes the Chinese display name of the api_key field to "密钥", and changes the display name of the field group to which this field belongs from "华炎云" to "华炎魔方".
...
First, let's create a new [package](./package) named locale-ja for Japanese internationalization in the Steedos project. Suppose the package is empty and the directory structure is as follows:
