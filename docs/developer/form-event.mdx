...
`select`类型即是选择框字段类型，该字段类型说明请参考文档 [字段类型索引](/docs/no-code/field_type) 中的 [选择框字段类型](/docs/no-code/field_type#%E9%80%89%E6%8B%A9%E6%A1%86%E5%AD%97%E6%AE%B5%E7%B1%BB%E5%9E%8B)。
...
很多情况下字段选项并不是固定的，而是需要请求后台接口来列出相关选项的，这时我们会把字段类型配置为 “相关表” 或 “主表子表”，其使用说明请参考 [字段类型索引](/docs/no-code/field_type)。
