---
title: 对象表格
description: Steedos微页面引擎，内置对象列表组件 ObjectTable，借助amis渲染器，可以帮助开发人员快速生成一个功能完善的对象表格组件。
---

这是一个基于Steedos对象的对象表格组件，它把对象记录显示为表格，同时包含了对象记录的增删改查以及其他操作按钮功能，与 [对象列表](./object_listview) 组件的主要区别是该组件可以配置表格显示的列、过滤条件等基本属性。
