...
- 公式：可以输入一段公式脚本来描述希望设置的动态默认值，详情请参 [数据映射](https://aisuda.bce.baidu.com/amis/zh-CN/docs/concepts/data-mapping) 或 [Amis表达式](https://aisuda.bce.baidu.com/amis/zh-CN/docs/concepts/expression) 。
​
### 非可编辑字段
字段不可编辑，包括以下几种情况：
...
只要字段不可编辑（这里指的是上面说的两种情况，即字段为readonly或隐藏状态），那么该字段上配置的默认值公式就不生效（但是默认值为固定值而不是公式的话还是能生效）。

需要注意的是，此时虽然默认值为固定值是有效的，但是必须用双引号把固定值括起来，否则不会生效。

如果想让不可编辑字段的默认值公式生效，请把该字段配置为“只读且编辑时显示”，即static，而不是“只读”（readonly），因为字段为readonly时默认值公式不会生效，此时字段默认值公式设置方法与上述可编辑字段是一样的。
