---
title: 公式函数索引
description: 在构建公式时使用这些运算符和函数。除非另行指定，否则所有的函数都可用在任何能够包含公式（例如，公式字段、验证规则、批准过程和工作流规则）的位置。
---
...
> 不要使用此函数进行空比较，如 MyDateTime__c == null。而应使用[ISBLANK](/docs/no-code/functions#isblank)。
...
■ 如果您希望使用“日期”字段，请使用[TODAY](/docs/no-code/functions#today)函数。
...
■ 如果您希望使用日期时间字段，请参阅[NOW](/docs/no-code/functions#now)函数。
...
> * 如果字段没有值，则使用 BLANKVALUE 函数返回指定置换表达式对应的值；如果您只想检查字段是否有值，则使用[ISBLANK](/docs/no-code/functions#isblank)函数。
> * 传入的两个参数值类型应该一样，否则公式将直接报错。
> * 传入的参数不支持布尔类型值，详情可参考：[使用布尔公式字段的提示](/docs/no-code/field_type#%E4%BD%BF%E7%94%A8%E5%B8%83%E5%B0%94%E5%85%AC%E5%BC%8F%E5%AD%97%E6%AE%B5%E7%9A%84%E6%8F%90%E7%A4%BA)。
> * 传入的参数不支持select字段类型值，详情可参考：[使用选项列表公式字段的提示](/docs/no-code/field_type#%E4%BD%BF%E7%94%A8%E9%80%89%E9%A1%B9%E5%88%97%E8%A1%A8%E5%85%AC%E5%BC%8F%E5%AD%97%E6%AE%B5%E7%9A%84%E6%8F%90%E7%A4%BA)。
...
<alert type="info">
■ 该函数与[ISBLANK](/docs/no-code/functions#isblank)很像，但是该函数会进一步返回字段值为空时的置换表达式对应的值，一般来说，如果您只是想判断字段值是否为空，应该使用后者而不是该函数。
...
<alert type="info">
* CASE函数中的任何参数都不支持布尔类型值，这不只表示第一个参数及后续用于比较值的其他参数不能是布尔类型值，最后一个参数也一样不支持布尔类型值，也就是说整个函数的返回值不可能是布尔类型值，详情可参考：[使用布尔公式字段的提示](/docs/no-code/field_type#%E4%BD%BF%E7%94%A8%E5%B8%83%E5%B0%94%E5%85%AC%E5%BC%8F%E5%AD%97%E6%AE%B5%E7%9A%84%E6%8F%90%E7%A4%BA)。
...
* IF函数不支持返回值为布尔类型的情况，即第二个及第三个参数不能是布尔类型，详情可参考：[使用布尔公式字段的提示](/docs/no-code/field_type#%E4%BD%BF%E7%94%A8%E5%B8%83%E5%B0%94%E5%85%AC%E5%BC%8F%E5%AD%97%E6%AE%B5%E7%9A%84%E6%8F%90%E7%A4%BA)。
...
■ 如果字段没有值，则使用 [BLANKVALUE](/docs/no-code/functions#blankvalue) 函数返回指定表达式；如果您只想检查字段是否有值，则使用 ISBLANK 函数。
...
<alert type="info">
■ 该函数只支持多选的选项列表字段，要使用单选选项列表字段请使用 [ISPICKVAL](/docs/no-code/functions#ispickval)。

■ 可以使用 [ISBLANK](/docs/no-code/functions#isblank) 决定选项列表字段是否为空。

■ 关于选项列表字段支持哪些其他函数，请参考 [使用选项列表公式字段的提示](/docs/no-code/field_type#%E4%BD%BF%E7%94%A8%E9%80%89%E9%A1%B9%E5%88%97%E8%A1%A8%E5%85%AC%E5%BC%8F%E5%AD%97%E6%AE%B5%E7%9A%84%E6%8F%90%E7%A4%BA)。
...
<alert type="info">
■ 该函数只支持单选的选项列表字段，要使用多选选项列表字段请使用 [INCLUDES](/docs/no-code/functions#includes)。
